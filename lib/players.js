...
export async function syncPlayers(db, matchId, playersA, playersB) {
  const incoming = [
    ...(playersA || []).map((p, i) => ({ team: 'A', number: p.number || '', position: p.position || '', name: (p.name || '').trim(), slot: p.slot || 'starter', sort_order: i })),
    ...(playersB || []).map((p, i) => ({ team: 'B', number: p.number || '', position: p.position || '', name: (p.name || '').trim(), slot: p.slot || 'starter', sort_order: i })),
  ].filter(r => r.name);
  ...
  for (const r of incoming) {
    ...
    if (match && !keptIds.has(match.id)) {
      ...
      const changed = match.number !== r.number || match.position !== r.position
        || match.sort_order !== r.sort_order || match.name !== r.name || match.slot !== r.slot;
      if (changed) {
        const { error } = await db.from('players')
          .update({ number: r.number, position: r.position, sort_order: r.sort_order, name: r.name, slot: r.slot })
          .eq('id', match.id);
